...
export interface ConnectionLayerHandle {
  updatePreview: (sourceId: string, sourceAnchor: AnchorPoint, x2: number, y2: number) => void;
  clearPreview: () => void;
}
...
export const ConnectionLayer = forwardRef<ConnectionLayerHandle, Props>(
  ({ connections, rectMap, width, height, series, theme, editMode, selectedConnectionId, onSelectConnection }, ref) => {
    const previewPathRef = useRef<SVGPathElement>(null);

    // Expose imperative handle so CanvasContainer can update the rubber-band
    // preview path on every mousemove without triggering a React re-render.
    useImperativeHandle(ref, () => ({
      updatePreview(sourceId, sourceAnchor, x2, y2) {
        const srcRect = rectMap.get(sourceId);
        const path = previewPathRef.current;
        if (!path || !srcRect) { return; }
        const src = anchorPixel(srcRect, sourceAnchor);
        path.setAttribute('d', `M ${src.x} ${src.y} L ${x2} ${y2}`);
    ...
    if (!connections.length && !editMode) {
      return null;
    }
    ...
    return (
      ...
      >
        ...
        {/* Rubber-band preview line — updated imperatively via ref to avoid re-renders on every mousemove */}
        <path
          ref={previewPathRef}
          visibility="hidden"
